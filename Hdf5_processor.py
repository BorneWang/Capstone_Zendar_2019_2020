...
class hdf5_processor:
    def __init__(self, src, goal, option = ''):
        # load files
        f = h5py.File(src,'r')
        self.aperture = f['radar']['squint_left_facing']['aperture2D']
    ...
    def increse_contrast(self):
        ...
        # increase contrast
        gamma = 0.57
        lin_coeff = 3.5 # Change this number (4.0, 3.0, 2.0)
        for i in range(len(self.keys)):
            ...
            heatmap = self.thresholding(heatmap, lin_coeff, 6)
    ...
    def load_backup_data(self):
        ...
        for i in list(np.linspace(0,(ite-1)*batch,ite).astype('int')):
            ...
            for j in keys_backup:
                ...
                self.max = max(self.max, np.nanmax(img[...]))
                self.min = min(self.min, np.nanmin(img[...]))
            f_backup.close()
